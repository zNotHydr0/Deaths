...
public class Break implements CommandExecutor {
    ...
    private void manageCommand(Player launcher, String[] args) {
        if (args.length == 0) {
            ...
            launcher.sendMessage("§7/break on");
            launcher.sendMessage("§7/break off");
            return;
        } else {
            if (args[0].equalsIgnoreCase("on")) {
                launcher.sendMessage("§c§lDEATHS §8| §aNow everyone can break blocks.");
                return;
            }
            if (args[0].equalsIgnoreCase("off")) {
                launcher.sendMessage("§c§lDEATHS §8| §4Now users can break blocks.");
                return;
            }
        }
    }
